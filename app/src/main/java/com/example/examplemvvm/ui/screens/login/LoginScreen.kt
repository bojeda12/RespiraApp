...
fun Login(
    ...
    )
    {
        ...
        TxtFieldGeneral(state.email,"Correo", "Ejemplo: pepe@gmail.com", errorMessage = emailError){ viewModel.onEvent(LoginEvent.EmailChanged(it)) }
        //TextFields(state.email, "Email", "Email", errorMessage = emailError) { viewModel.onEvent(LoginEvent.EmailChanged(it)) }
        Spacer(modifier = Modifier.padding(10.dp))
        TxtPasswordField(state.password,"Contrasena", "Ingresa tu contrasena", errorMessage = passwordError) {viewModel.onEvent(LoginEvent.PasswordChanged(it)) }
